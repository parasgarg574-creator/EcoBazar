...
const Category = () => {
  ...
  const [filters, setFilters] = useState({ image: "all" });
  const canEditCategories = permissions.isAllowed("updateCategories");
  const canDeleteCategories = permissions.isAllowed("deleteCategories");
  ...
  const handleViewCategory = async (categoryId) => {
    ...
    try {
      const response = await apimethods.getApi(`/getSingle/${categoryId}`);
      const category = response?.data?.data || response?.data || response;
      Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            ...
            ${
              category?.image
                ? `<img src="${category.image}" alt="${category.name || ""}" style="width:100%;max-height:200px;object-fit:cover;border-radius:8px;margin-top:10px;" />`
                : ""
            }
          </div>
        `,
  ...
  const handleEditCategory = async (categoryId) => {
    ...
    try {
      ...
      console.error("Failed to get category for edit:", error);
      Swal.fire({
  ...
  const handleDeleteCategory = async (categoryId) => {
    ...
    try {
      await apimethods.deleteApi(`/deleteCategory/${categoryId}`);
      setCategories((prev) =>
  ...
  const filteredCategories = useMemo(() => {
    return categories.filter((category) => {
      ...
      const matchesImage =
        !filters.image ||
        filters.image === "all" ||
        (filters.image === "with" && Boolean(category?.image)) ||
        (filters.image === "without" && !category?.image);
